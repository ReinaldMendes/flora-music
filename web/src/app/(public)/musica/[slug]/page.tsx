...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const album = await fetch(`${API}/albuns/${params.slug}`).then(r => r.json())
    return { title: album.title, description: album.description || `Álbum ${album.title} de Flora.` }
  } catch { return { title: 'Álbum' } }
}

export default async function AlbumPage({ params }: Props) {
  let album: any
  try { album = await fetch(`${API}/albuns/${params.slug}`, { next: { revalidate: 3600 } }).then(r => r.json()) }
  catch { notFound() }
    ...
    <>
      <div className="pt-24 bg-flora-deep">
        <div className="container-flora grid md:grid-cols-2 gap-12 pb-16 items-end">
          <AnimatedSection>
            <div className="relative aspect-square max-w-sm rounded-lg overflow-hidden">
            ...
            {album.streamingLinks && (
              <div className="flex flex-wrap gap-3">
                {Object.entries(album.streamingLinks).map(([platform, url]: any) => (
                  <a key={platform} href={url} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 bg-flora-cream/10 border border-flora-cream/20 text-xs font-body text-flora-cream/65 rounded capitalize hover:bg-flora-cream hover:text-flora-deep transition-all duration-300">{platform}</a>
                ))}
              </div>
            )}
          </AnimatedSection>
