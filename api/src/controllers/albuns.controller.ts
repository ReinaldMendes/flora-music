...
export async function getAll(req: Request, res: Response) {
  ...
  const albums = await prisma.album.findMany({
    ...
    include: { tracks: { orderBy: { trackNumber: 'asc' } }, credits: true },
  })
...
export async function create(req: Request, res: Response) {
  try {
    ...
    const album = await prisma.album.create({
      data: { title, slug: slugify(title), releaseDate: new Date(releaseDate), coverUrl, description, type, streamingLinks, featured: !!featured, published: !!published },
    })
